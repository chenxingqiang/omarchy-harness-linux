...
The L1 table is encoded in `harness/lib/mutations.js` as a closed catalog and exposed as `dispatch.write`. Each method maps to a typed Omarchy effector argv. `dispatch.write.execute`, `dispatch.write.shell`, and generic `dispatch.system.execute` remain absent. Unknown names and held L2 ops fail closed as `L2_UNREPRESENTABLE`.
...
### Phase 3 L2 — opened: finite system write, still no generic execute

Phase 3 L2 is a new object, not a widening of `dispatch.write`. `dispatch.system` is a frozen method table:

```text
dispatch.system.pkg.add
dispatch.system.pkg.drop
dispatch.system.update
dispatch.system.snapshot.create
dispatch.system.snapshot.restore
dispatch.system.system.reboot
...
Still open (not the Session Control Plane MVP. Rev 9 executes the L2 v0 table. Do not reopen Control Plane / Data Plane / Session Log / Overlay):
...
3. **Exact `dsh` version string** for the first vendored bundle. Resolved: `integrity.json` pins `@deepseek-ai/dsh@0.1.1-rc.2` (commit `b150a551b8d465e31e418e1b2eaf5e79bbb7d28e`, tag `dsh-v0.1.1-rc.2`), with `harness/package.json` + `harness/package-lock.json` as the real, `npm ci`-installable packaging artifact (`node_modules` stays uncommitted, installed into `/usr/lib/omarchy-harness/` at package build time). `harness/bundle/dsh-omarchy/` is the `dsh-omarchy` Cordis bundle from the Composition section above: it registers the existing `harness/lib/omarchy-harness.js` dispatch surface as real `ctx.tools` entries (via `@deepseek-ai/dsh-tools`'s `defineTool`/`ctx.tools.register`) rather than re-implementing it, keeping L2 approval on this repo's own ask/snapshot/audit pipeline rather than `dsh-user-approval`. `bin/omarchy-harness-dsh-link` materializes `$DSH_HOME/profiles/omarchy/` (bundles: `dsh-base`, `@omarchy/dsh-omarchy`, optionally `dsh-web-app` via `--with-web`) and backs up `$DSH_HOME/{sessions,profiles/omarchy}` before any pin change, covering rc.8's incompatible session-query SQLite format. `test/shell.d/harness-dsh-omarchy-bundle-test.sh` boots the bundle against the real installed `@deepseek-ai/cordis` + `@deepseek-ai/dsh-tools` (skips cleanly when `harness/node_modules` is absent).
